  ...
  return (
    <div style={{ display: "flex", alignItems: "flex-end", gap: 2, height: 36, marginTop: 4 }}>
      {Array.from({ length: bars }).map((_, i) => {
        ...
        const h = active
          ? base + Math.abs(Math.sin(i * 2.3)) * 28
          : base + Math.abs(Math.sin(i * 1.7)) * 5;
        return (
          <div key={i} style={{
            ...
            <div style={{ marginTop: 4 }}>
              ...
              <div style={{ background: "var(--color-border-tertiary)", borderRadius: 4, height: 5, overflow: "hidden" }}>
                <div style={{
                  ...
                  background: confidence >= 90 ? "#E24B4A" : "#EF9F27",
      ...
      {logs.map((l, i) => (
        <div key={i} style={{ display: "flex", gap: 8, marginBottom: 6, fontSize: 12 }}>
          ...
          <span style={{
            ...
            background: l.type === "ALERT"  ? "#FCEBEB"
                      : l.type === "CLEAR"  ? "#EAF3DE" : "#E6F1FB",
            color:      l.type === "ALERT"  ? "#A32D2D"
                      : l.type === "CLEAR"  ? "#3B6D11" : "#185FA5",
          }}>{l.type}</span>
...
export default function EchoGuardSimulation({ settings = {} }) {
    ...
    chain.forEach((toId, i) => {
      ...
      scheduleTimeout(() => {
        ...
        if (toId !== "GW") {
          setNodeStates(prev => ({
            ...prev,
            [toId]: { status: "relaying", threat, confidence: null, pulse: false },
          }));
          ...
          addLog("ALERT", `✅ Gateway received alert — response time: ${(ms / 1000).toFixed(1)}s`);
          setStats(prev => {
            const times = [...prev.responseTimes, ms];
            return {
              ...prev,
              avgResponse: Math.round(times.reduce((a, b) => a + b, 0) / times.length),
              responseTimes: times,
            };
          });
          ...
          scheduleTimeout(() => {
            ...
            PIPELINE_NODES.forEach(n => {
              setNodeStates(prev => ({
                ...prev,
                [n.id]: { status: "idle", threat: null, confidence: null, pulse: false },
              }));
            });
  ...
  const triggerThreat = useCallback((nodeId) => {
    ...
    setNodeStates(prev => ({
      ...prev,
      [nodeId]: { status: "detecting", threat, confidence: conf * 0.3, pulse: true },
    }));
    ...
    const rampUp = setInterval(() => {
      ...
      if (progress >= 1) {
        ...
        if (isVandalism) {
          setNodeStates(prev => ({
            ...prev,
            [nodeId]: { status: "alert", threat, confidence: conf, pulse: true },
          }));
          ...
          setNodeStates(prev => ({
            ...prev,
            [nodeId]: { status: "idle", threat: null, confidence: null, pulse: false },
          }));
  ...
  const triggerAmbient = useCallback(() => {
    ...
    setNodeStates(prev => ({
      ...prev,
      [nodeId]: { status: "detecting", threat: null, confidence: 55 + Math.random() * 20, pulse: false },
    }));
    scheduleTimeout(() => {
      setNodeStates(prev => ({
        ...prev,
  ...
  return (
    <div style={{ padding: window.innerWidth <= 768 ? "12px 10px" : "16px 14px", fontFamily: "var(--font-sans)" }}>
        ...
        <svg viewBox={`0 0 ${svgW} ${svgH}`} width="100%" style={{ overflow: "visible", maxHeight: window.innerWidth <= 768 ? 250 : 300 }}>
          ...
          {PIPELINE_NODES.map((n, i) => {
            ...
            return (
              <g key={n.id + "_line"}>
                <line
                  x1={n.x + 20} y1={nodeY + 14} x2={endX} y2={nodeY + 14}
          ...
          <g>
            <rect x={GATEWAY.x} y={nodeY - 4} width={46} height={36} rx={6}
              ...
              stroke={gwActive ? "#378ADD" : "#B4B2A9"}
              strokeWidth={gwActive ? 2 : 1} />
      ...
      <div style={{ display: "flex", gap: 10, marginTop: 14, flexWrap: "wrap" }}>
        ...
        {PIPELINE_NODES.map(n => (
          <button key={n.id} onClick={() => manualTrigger(n.id)} style={{ fontSize: 12 }}>
            Trigger {n.id}
          </button>
        ))}
        <button
          onClick={() => {
            setLogs([]);
            setStats({ detected: 0, ambient: 0, alerts: 0, avgResponse: 0, responseTimes: [] });
          }}
